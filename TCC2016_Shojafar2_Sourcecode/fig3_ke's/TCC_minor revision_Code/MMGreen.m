...
close all
%%%%%%%%%%%%%%%%%%%%%%%%difinite variables
M=700;% numbrs of virtual machines
M_vector=[700:50:800];
...
k_e=[0.005;0.15;0.5]*100;% Joule/(Mega Hz)^2 % scenario 1
...
%    expression f_zero(I);% starting frequncy of each VM
   ...
   %expressions temp_ss(M,Q+1) t_opt_temp(M,Q) t_opt_sorted(M,Q+1);
    ...
    minimize ((omega(1)*sum(PowerFunc(:).*t_opt(:)))+((T_t-T(1))*((Zeta5*sum((power(2,2.*((sum(F(:,2:Q+1).*t_opt(:,2:Q+1),2))/(T_t-T(1))*W(1)))-1)))+sum(P_Idle(:)))));
...
%     test=issorted(t_opt_sorted, 'rows');
    ...
    % CPU_energy1(tt,i,j)=Opt_Comp_Energy(i,j);
      ...
      Opt_reconfig_Energy12(i,jj)=k_e(2)*Opt_reconfig_Energy1(i,jj);
      Opt_reconfig_Energy13(i,jj)=k_e(3)*Opt_reconfig_Energy1(i,jj);
%      Opt_reconfig_Energy13(jj)=k_e(3)*sum(Opt_reconfig_Energy1(i));
    %Opt_Comm_Energy(i,j)=(T_t-T(1))*((Zeta(2)*sum((power(2,2.*((sum(F(:,2:Q+1).*t_opt(:,2:Q+1),2))/(T_t-T(1))*W(1)))-1)))+sum(P_Idle(:)));
    RTT=70;
    % scenario 1
    Opt_Comm_Energy1(i,jj)=(sum(Zeta5*(RTT.*2.*((sum(F(:,2:Q+1).*t_opt(:,2:Q+1),2))/(T_t-T))).^2)+sum(P_Idle(:)));
    ...
    total_energy2(i,jj)=Opt_Comp_Energy(i,jj)+Opt_reconfig_Energy12(i,jj)+Opt_Comm_Energy1(i,jj);
    total_energy3(i,jj)=Opt_Comp_Energy(i,jj)+Opt_reconfig_Energy13(i,jj)+Opt_Comm_Energy1(i,jj);
...
%     Opt_reconfig_Energy11(jj)=k_e(1)*sum(Opt_reconfig_Energy1);
%     Opt_reconfig_Energy12(jj)=k_e(2)*sum(Opt_reconfig_Energy1);
%     Opt_reconfig_Energy13(jj)=k_e(3)*sum(Opt_reconfig_Energy1);
%     
%     for j=1:Q+1
%         %TT_Q(i)=mean(TT1(:,j));
...
tot_reconfig_Energy2=zeros(1,length(MM));
tot_reconfig_Energy3=zeros(1,length(MM));
...
for jj=1:length(MM)
    ...
    tot_reconfig_Energy2(jj)=mean(Opt_reconfig_Energy12(:,jj));
    tot_reconfig_Energy3(jj)=mean(Opt_reconfig_Energy13(:,jj));
...
save('Myapproach_energyScnerioN100F1.mat','total_energyt1','total_energyt2','total_energyt3','tot_reconfig_Energy1',...
     'tot_reconfig_Energy2','tot_reconfig_Energy3','Opt_Comp_Energyt1','Opt_Comm_Energyt1','cvx_cputime_2'); 
...
legend('$k_e=0.005$$(Joule/(MHz)^2)$','$k_e=0.05$$(Joule/(MHz)^2)$','$k_e=0.5$$(Joule/(MHz)^2)$','Interpreter','latex','FontSize',40);
grid on

figure(2001) %fig 3b
plot(1:1:length(MM),tot_reconfig_Energy1,'-b',1:1:length(MM),tot_reconfig_Energy2,'--k',1:1:length(MM),tot_reconfig_Energy3,':r');
xlabel('$VMs$','Interpreter','latex','FontSize',40);
