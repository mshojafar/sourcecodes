...
%    expression f_zero(I);% starting frequncy of each VM
   ...
   %expressions temp_ss(M,Q+1) t_opt_temp(M,Q) t_opt_sorted(M,Q+1);
    ...
    %minimize ((omega(1)*sum(PowerFunc(:).*t_opt(:)))+((T_t-T(1))*((Zeta5*sum((power(2,2.*((sum(F(:,1:Q+1).*t_opt(:,1:Q+1),2))/(T_t-T(1))*W(1)))-1)))+sum(P_Idle(:)))));
   ...
   % (2*(T_t-T(1))).*(sum(F(kk,:).*t_opt(kk,:))./(sum(F(kk,:).*t_opt(kk,:),2)))+T(1)-T_t<=0;
    ...
    sum((sum(F(:,1:Q+1).*t_opt(:,1:Q+1),2))/(T_t-T(1)))-R_tot<=0;
...
save('MMGreen_ScenarioM50_5_100N100F1T3.mat','total_energyt1','tot_reconfig_Energy1',...
        'Opt_Comm_Energyt1','Opt_Comp_Energyt1','Total_Time','time_meanWL_M', 'time_meanWL');

    xlswrite('MMGreen_total_energy_Real.xlsx',total_energyt1);
    csvwrite('MMGreen_total_energy_Real.dat',total_energyt1);
    
    xlswrite('MMGreen_tot_reconfig_Energy_Real.xlsx',tot_reconfig_Energy1);
    csvwrite('MMGreen_tot_reconfig_Energy_Real.dat',tot_reconfig_Energy1);
    
    xlswrite('MMGreen_tot_Comm_Energy_Real.xlsx',Opt_Comm_Energyt1);
    csvwrite('MMGreen_tot_Comm_Energy_Real5.dat',Opt_Comm_Energyt1);
    
    xlswrite('MMGreen_tot_Comp_Energy_Real.xlsx',Opt_Comp_Energyt1);
    csvwrite('MMGreen_tot_Comp_Energy_Real.dat',Opt_Comp_Energyt1);
    
    
xlswrite('MMGreen_time_T_T_tperQ_Real.xlsx',time_meanWL_M);
csvwrite('MMGreen_time_T_T_tperQ_Real.dat',time_meanWL_M);

%xlswrite('Time.xlsx',Total_Time);
%csvwrite('csvtime.dat',Total_Time);
time=toc;

Results=load('MMGreen_ScenarioM50_5_100N100F1T3.mat','total_energyt1','tot_reconfig_Energy1',...
        'Opt_Comm_Energyt1','Opt_Comp_Energyt1','Total_Time','time_meanWL_M', 'time_meanWL');    
