function [] = computationResult(fAP2C,fAANT,fATCAGA,iteration,dataSetNum)
    ...
    while(counter <= iteration)
        ...
        m = 1;
        while(m <= dataSetNum)
            secondAddress = strcat(num2str(counter),'\','comp',num2str(m),'.mat');
            ...
            rCompP2C(counter,m) = sum(compP2C.comp,2);
            rCompANT(counter,m) = sum(compANT.comp,2);
            rCompTCAGA(counter,m) = sum(compTCAGA.comp,2);
